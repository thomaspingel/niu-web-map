...
function searchOSM() {
	...
	if (value == "" || typeof(value) == "undefined") {
		...
		//getJSON fails quietly on IE 8/9 - use ajax method instead?
		...
		var strSearch = strAPI+query+viewbox+poly+addrDet;
		
		//call string, read in json results.  
		jQuery.getJSON(strSearch,  function(data) {
			var strPoly = "";
			var arrayPoly = [];
			var items = []; 
			markerArray = [];
			...
			if (items.length == 1) {
				...
				$('<p>', { html: "No results found for " + "\""+ value + "\"" }).appendTo('#results');
			}
...
function getOSMObj(osmID, center) {
	...
	if(osmID !== "") {
		...
		jQuery.getJSON(overpassJSON, function(data) {
			...
			/* need a way to seperate multiple values eg ';' found, use split() maybe?
			   Also turn www link into clickable link
			   which key values and which order?
				example tags
				"Wifi": "Yes",
				...
				"alt_name": "Geography Department; Geology Department and Environmental Geosciences Department,  Meteorology Department;",
				"amenity": "university",
				"building": "public",
				"name": "Davis Hall",
